...
function updateCookieFromCheckbox(id) {
  var name = $(id).attr("id");
  if (name.indexOf("-") !== -1) {
    // Replaced includes with indexOf
...
updatePage = function (data) {
  ...
  if (data.title != data.libraryName) {
    if (data.libraryId) {
      document.title =
        data.libraryName + " " + data.libraryId + " - " + data.title;
    } else {
...
function link_Clicked(event) {
  ...
  if (!isPushStateEnabled || currentData.page === "customize") {
    return;
  }
  // let read, fetch, zippper etc. do their thing
  var url = currentLink.attr("href");
  if (
    url.indexOf("/read/") !== -1 || // Replaced includes with indexOf
    url.indexOf("/fetch/") !== -1 ||
  ...
  // if the link has a download attribute let the browser handle it directly
...
function search_Submitted(event) {
  ...
  event.preventDefault();
  var url = str_format(
    currentData.baseurl + "?page=query&current={0}&query={1}&db={2}",
    currentData.page,
...
function initiateAjax(url, theme, templates, version) {
  ...
  $.when(
    $.get({
      url: templates + "/" + theme + "/header.html?v=" + version,
      cache: true,
    }),
    $.get({
      url: templates + "/" + theme + "/footer.html?v=" + version,
      cache: true,
    }),
    $.get({
      url: templates + "/" + theme + "/bookdetail.html?v=" + version,
      cache: true,
    }),
    $.get({
      url: templates + "/" + theme + "/main.html?v=" + version,
      cache: true,
    }),
    $.get({
      url: templates + "/" + theme + "/page.html?v=" + version,
      cache: true,
    }),
    $.get({
      url: templates + "/" + theme + "/suggestion.html?v=" + version,
      cache: true,
    }),
    $.getJSON(url),
  )
    ...
    .fail(function (error) {
      if (error.responseText) {
        ...
        document.write(
          'Error loading templates from directory "' +
            encodeURI(templates + "/" + theme) +
            '/" for url: ' +
            encodeURI(url),
        );
      }
